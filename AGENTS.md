...
- **Smoke test faune (vache / natation)** — scène dédiée, sans monde complet ; écrit `artifacts/smoke_vache_natation.log` et imprime `SMOKE_VACHE_NATATION_RESULT=OK|ECHEC` :
  ```bash
  godot --path . res://Tests/SmokeVacheNatation.tscn --headless
  ```
  (Nécessite Godot **.NET** 4.6 sur la machine ; le dépôt active un mode test sur `BoeufSauvage` le temps du run.)

- **Exploration vidéo automatique du monde** — charge `monde_zero.tscn`, attend le spawn prêt puis pilote une caméra cinématique pendant ~60s :
  ```bash
  godot --path . res://Tests/ExplorationAutoMonde.tscn --write-movie "artifacts/exploration_monde.avi" --fixed-fps 30
  ```
  (Scène de test isolée : n'altère pas le gameplay normal.)
